...
* `reg8`, `reg16`, `reg32`, `reg64`: register types — plain unsigned integers
  that are also implicitly `volatile`, for memory-mapped hardware registers.
* `void`: same as in C.

For convenience, the __char__ keyword is also available, and is identical to the
__u8__ type. It's intended to be used only for strings, to keep that use visually
distinct from raw byte data.

C2 has __no__ type specifiers like __signed__, __unsigned__, __long__ or __short__
— each width and signedness combination simply has its own named type.

### Built-in type fields
Built-in primitive types expose their properties as fields — minimum and maximum
value, bit width, and, for floating-point types, NaN and infinity.

Each integral type has a minimum value, a maximum value and a bit width:

* `char.min` is 0, `char.max` is 255, `char.width` is 8;
* `i8.min` is -128, `i8.max` is 127, `i8.width` is 8;
* `i16.min` is -32768, `i16.max` is 32767, `i16.width` is 16;
* `i32.min` is -2147483648, `i32.max` is 2147483647, `i32.width` is 32;
* `i64.min` is -9223372036854775808, `i64.max` is 9223372036854775807, `i64.width` is 64;
...
Pointer types are created by adding an asterisk (`*`) after the type they point
to:
...
The `nil` keyword is used for a null pointer value:
```c
char* name = nil;
```
...
// the size can also be inferred from an initializer instead of given explicitly
void*[] c = { nil, nil }
```
